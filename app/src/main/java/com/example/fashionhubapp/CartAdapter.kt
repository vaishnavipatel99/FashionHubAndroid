...
class CartAdapter(
    ...
    override fun onBindViewHolder(
        ...
        holder.name.text = product?.productName ?: "N/A"
        holder.price.text = "₹ $price"
        holder.qty.text = item.quantity.toString()
        holder.total.text = "Total: ₹ ${price * item.quantity}"

        val imageName = product?.productImage?.replace("/images/", "") ?: ""
        val imageUrl = "http://10.0.2.2:5041/images/$imageName"
        ...
        holder.remove.setOnClickListener {
            RetrofitClient.instance.deleteCart(item.cartId)
    ...
    private fun updateCart(item: Cart) {
        val req = CartRequest(
            uid = item.uid,
            pid = item.pid,
        ...
        RetrofitClient.instance.updateCart(item.cartId, req)
            .enqueue(object : Callback<Any> {
                override fun onResponse(call: Call<Any>, response: Response<Any>) {}
                override fun onFailure(call: Call<Any>, t: Throwable) {}
            })
